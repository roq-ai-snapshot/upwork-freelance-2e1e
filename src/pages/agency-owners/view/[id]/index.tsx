...
import { useAgencyOwnerFindFirst, useRoqClient } from 'lib/roq';

import { AgencyOwnerInterface } from 'interfaces/agency-owner';

function AgencyOwnerViewPage() {
  ...
  const { data, error, isLoading } = useAgencyOwnerFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('agency_owner', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/agency-owners/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Joining Date"
                  text={data?.joining_date ? format(data?.joining_date, 'dd-MM-yyyy') : ''}
                />

                <FormListItem
                  label="Leaving Date"
                  text={data?.leaving_date ? format(data?.leaving_date, 'dd-MM-yyyy') : ''}
                />
