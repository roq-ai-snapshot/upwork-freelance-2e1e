...
import { useRoqClient, useAgencyOwnerFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<AgencyOwnerInterface, unknown>;

interface AgencyOwnerListPageProps {
...
export function AgencyOwnerListPage(props: AgencyOwnerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useAgencyOwnerFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.agency_owner.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: AgencyOwnerInterface) => {
    if (hasAccess('agency_owner', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/agency-owners/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.joining_date ? format(record?.joining_date, 'dd-MM-yyyy') : '',
    },
    {
      id: 'leaving_date',
      header: 'Leaving Date',
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.leaving_date ? format(record?.leaving_date, 'dd-MM-yyyy') : '',
    },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/agency-owners/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('agency_owner', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/agency-owners/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('agency_owner', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="agency_owner" />
        </Flex>

        {hasAccess('agency_owner', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/agency-owners/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
