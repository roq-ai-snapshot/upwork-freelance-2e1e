...
import { useRoqClient, useAgencyOwnerFindFirst } from 'lib/roq';
...
function AgencyOwnerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAgencyOwnerFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AgencyOwnerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.agency_owner.update({
        data: values as RoqTypes.agency_owner,
        where: {
          id,
        },
      });
