...
import { useRoqPlatformUserProfiles, useAgencyMemberCount, useAgencyOwnerCount, useFreelancerCount } from 'lib/roq';

import { AgencyOwnerListPage as ShowCaseList } from 'pages/agency-owners';

function DashboardPage() {
  const { session } = useSession();

  const agencyMemberStat = useAgencyMemberCount();

  const agencyOwnerStat = useAgencyOwnerCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      ...
      result: agencyMemberStat?.data,
      isLoading: agencyMemberStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Agency Owners',
      result: agencyOwnerStat?.data,
      isLoading: agencyOwnerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Freelancers',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Agency Owners
                </Text>
                <NextLink href={`agency-owners`} passHref legacyBehavior>
                  <Button
