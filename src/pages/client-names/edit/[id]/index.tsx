...
import { useRoqClient, useClientNameFindFirst } from 'lib/roq';
...
function ClientNameEditPage() {
  ...
  const { data, error, isLoading, mutate } = useClientNameFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ClientNameInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.client_name.update({
        data: values as RoqTypes.client_name,
        where: {
          id,
        },
      });
